...
- **iPhone** (iOS 17+): `HitCam-ios-unsigned.ipa` со страницы Releases ставится с Windows через Sideloadly — см. [docs/install-ios.md](docs/install-ios.md).
...
В панели справа две вкладки. **«Камера»** — настройки камеры телефона и обработка картинки: переключатель
**«Обработка»** в заголовке выключает её целиком (ползунки сохраняют значения). **«Эксперименты»** — шумодав NVIDIA,
анализ объектов, руки и скрытие лиц: у каждой функции свой переключатель в заголовке, а общий переключатель вкладки
выключает их все сразу и включает обратно как было.

Обработка считается на видеокарте ПК (Direct3D 11, любая видеокарта; без неё — программный режим) после
декодирования, телефон не нагружает. Когда всё выключено, кадры проходят без изменений.
...
- **Удаление артефактов сжатия (NVIDIA RTX)**, вкладка «Эксперименты» — нейросеть NVIDIA «Artifact Reduction» убирает квадраты, ореолы и
...
- **Шумоподавление камеры телефона** (Выкл / Быстрое / Качество) — шум убирает сама камера телефона ещё до сжатия, это
  даёт самую чистую картинку. Есть на Android (если камера даёт выбор), по умолчанию «Быстрое»: «Качество» на части
  телефонов снижает 60 кадров/с. iOS управлять этим не позволяет.

**Улучшение картинки:** свежий вид «как с камеры» одним переключателем — адаптивная резкость (подчёркивает контуры,
не трогая ровные места, где шум и блоки сжатия), объём (локальный контраст без ореолов) и сочность (блёклые цвета,
тон кожи почти не меняется). Сила и каждая часть настраиваются, меньше 1 мс на кадр на любой видеокарте. Лучше всего
вместе с шумодавом NVIDIA.
...
Раздел **«Анализ объектов»** на вкладке «Эксперименты» находит в кадре 80 видов объектов (люди, животные, транспорт, предметы)
...
рамки в само видео — их увидят в Zoom, Discord, OBS. «Показывать подписи» убирает подписи и в превью, и в камере —
остаются только рамки.
...
Раздел **«Отслеживание рук»** на вкладке «Эксперименты» ставит точки на суставы и кончики пальцев, до двух рук сразу.
Точки держатся на пальцах и сглаживаются, чтобы не дрожать; переключатель «Соединять точки линиями» рисует «скелет»
руки. Точки видны в превью, в камеру — по желанию (см. ниже). Работает на ПК: модели **MediaPipe Hands** (Google, Apache 2.0, в ONNX от OpenCV Zoo) через ONNX Runtime +
...
Раздел **«Скрытие лиц»** на вкладке «Эксперименты» находит лица и закрывает их мозаикой, размытием или заливкой
выбранного цвета (сила — ползунком). Новое лицо скрыто сразу; клик по лицу в превью открывает его, повторный — снова
скрывает. Кого открыли, HitCam узнаёт по лицу, даже если человек вышел из кадра и вернулся; отпечатки лиц хранятся
только в памяти и забываются при закрытии программы («Скрыть всех» забывает их сразу). Если лицо на миг потерялось
(поворот головы, рука перед лицом), эффект держится на месте ещё около секунды. Пока анализ ещё не знает, где лица
(модели загружаются, телефон только подключился, сменился объектив или ориентация, сбой видеокарты), закрыт весь кадр;
если анализ отстаёт, лица остаются закрытыми на прежних местах, а не открываются. Двойной клик по превью (на весь
экран) лица не переключает.

Лица скрыты в превью и, если не выключено, в камере «HitCam»; рамки вокруг лиц в камере — по желанию. Работает на ПК:
детектор **YuNet** (MIT) и распознавание **SFace** (Apache 2.0) из OpenCV Zoo через ONNX Runtime + DirectML, около
4 мс на кадр и 2 мс на новое лицо на RTX 3070. Модели входят в архив релиза (`models\faces`). Подробнее:
[docs/vision.md](docs/vision.md#скрытие-лиц).

## Если что-то сломалось

Журнал HitCam для ПК — `%LOCALAPPDATA%\HitCam\logs`: `hitcam.log` (запуск, подключения, ошибки; старый — `hitcam.1.log`)
и `native-crash.log` (сбои внутри нативных библиотек: модуль, смещение, стек). Приложите их к сообщению об ошибке —
PIN, ключи и картинка туда не пишутся. Настройки лежат в `%APPDATA%\HitCam\settings.json`.
...
модели из OpenCV Zoo — MediaPipe Hands (Apache 2.0), YuNet (MIT), SFace (Apache 2.0),
ONNX Runtime (MIT), DirectML.dll (Microsoft, распространяется по лицензии Microsoft для DirectML), Avalonia, SkiaSharp,
ReactiveUI, QRCoder (MIT), заголовки NVIDIA Video Effects SDK (MIT; сама среда NVIDIA ставится отдельно).
Тексты лицензий лежат в архиве релиза рядом с `HitCam.exe`: `THIRD-PARTY-NOTICES.txt` и папка `licenses`.
