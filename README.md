...
на RTX 3070 около 5–6 мс на кадр 1080p. Уровень шума измеряется в каждом кадре: на чистой картинке обработка
выключается сама (нейросеть там только портит детали), при заметном шуме плавно включается мягкая модель,
сильная — лишь при очень сильном шуме. Ползунок «Сила» задаёт верхний предел.
